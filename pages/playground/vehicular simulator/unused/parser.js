...
let Grib2Message = function(sections) {
...
let parseIndicator = function(binary) {
    let rv = {};
...
let parseSections = function(binary) {
    let rv = [], section;
...
let parseSection = function(binary) {
...
let dataRepresentationTemplateParsers = {
...
let gridParsers = {
    ...
    40: function(binary) { let rv={};
        ...
        rv.scanningMode = binary.read('uint8');
        let basicAngle = ((rv.basicAngle == 0) || (rv.basicAngle == 0xffffffff)) ? 1 : rv.basicAngle;
        let scale = 1e-6/basicAngle;
        rv.la1 *= scale; rv.lo1 *= scale;
        rv.la2 *= scale; rv.lo2 *= scale;
        rv.di *= scale;
        return rv;
